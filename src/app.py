...
def signup_for_activity(activity_name: str, email: str):
    ...
    if email.startswith("teacher@"):
        raise HTTPException(status_code=400, detail="Email cannot be a teacher email")
    # Validate email is not a staff
    if email.startswith("staff@"):
        raise HTTPException(status_code=400, detail="Email cannot be a staff email")
    # Validate email is not a parent                            
    if email.startswith("parent@"):
        raise HTTPException(status_code=400, detail="Email cannot be a parent email")
    # Add student
